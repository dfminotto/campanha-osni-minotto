...
export async function POST(request: NextRequest) {
  try {
    ...
    const { error: profileCreateError } = await supabaseAdmin
      .from('user_profiles')
      .insert([{ id: newUser.user.id, email: email.toLowerCase(), role }])
    ...
    return NextResponse.json(
      { success: true, message: 'Usuário criado com sucesso', user: { id: newUser.user.id, email: newUser.user.email, role } },
      { status: 201 }
    )
  } catch (error) {
